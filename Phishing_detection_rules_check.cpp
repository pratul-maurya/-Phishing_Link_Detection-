...
int isSubstring(string s1, string s2)//function to compare strings
{
    int M = s1.length();
    int N = s2.length();
...
{   
    ...
    cout<<"Enter URL to be checked(Enter with https in lowercase): ";
    cin>>url;
    cout<<url<<endl;
    ...
    if(flag==0)
    {
        if(susp==1)
        {
            cout<<"Given URL is a suspicious link";
        }
        else if(susp !=1)
        cout<<"Given URL is legitimate\n";
    }
    else if(flag>0)
    cout<<"Given URL is a phishing link";
    cout<<"\nNumber of flags raised: "<<flag;
    return 0;
}